...
void s_load () {
 ...
 if (loaded) printf("ERROR:There is already an object program loaded in
memory!\n");
 else
 {
 f = fopen( fname, "r" );
...
int i, j=0;
 ...
 printf("%x ", curr_add);
 for (i=0+j*32; i<8+j*32 && memory[i] != '\0'; i++) printf("%c",
memory[i]);
 printf(" ");
...
void s_run (){
 ...
 break;
 case oCOMP: //compare
 if (reg_A > get_value(operand, indexed)) reg_SW = gt;//greater
than
 else if (reg_A < get_value(operand, indexed)) reg_SW = lt;// less
than
 else reg_SW = eq;
 break;
 case oDIV: //div
 ...
 break;
 case oJEQ: //jeq
 if (reg_SW == eq) reg_PC = operand, curr_add = (reg_PC -
start_add) * 2;
 break;
 case oJGT: //jgt
 if (reg_SW == gt) reg_PC = operand, curr_add = (reg_PC -
start_add) * 2;
 break;
 case oJLT: //jlt
 if (reg_SW == lt) reg_PC = operand, curr_add = (reg_PC -
start_add) * 2;
 break;
 case oJSUB: //jsub
 reg_L = reg_PC, reg_PC = operand, curr_add = (reg_PC - start_add) *
2;
 break;
 case oLDA: //lda
 reg_A = get_value(operand, indexed);
